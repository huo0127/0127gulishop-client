...
![image-20210723152401645](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210723152401645.png)
...
```
	...
	images		在對應的組件目錄下, 創建images, 把對應的圖片撈到這個				文件
				夾下(注意路徑要對應好)
...
原来的是使用 css 去做的，咱们不用
添加移入和移出事件（关键是数据的设计）
移入哪一个把哪一个的 index，传到回调函数，然后把 currentIndex = index
上面使用类的对象写法：item_on : currentIndex == index
而移出事件我们需要移出全部分类的时候才会消失，因此移出事件我们需要添加在外部一个 div 上
