        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 
                  輸入框的blur事件，是失去焦點事件，只要添加了它，失去焦點就會觸發這個事件。
                  但是不好，因為不不會去判斷這次輸入的數據和之前的數據是不是一樣再觸發。
                  輸入框的change事件內部包含失去焦點事件，當用戶失去焦點的時候，也會選擇性觸發change事件，觸發這個事件前提是當前這個數據，必須和之前的數據不一樣，才能觸發，否則不觸發。
                 -->
                <input
                  autocomplete="off"
                  class="itxt"
              ...
              <div class="add">
                <!-- 
                  之前我們的跳轉幾乎都是點擊直接跳轉，因為我們在跳轉之前不需要發請求，
...
export default {
  ...
  methods: {
    ...
    changeChecked(spuSaleAttrValueList, spuSaleAttrValue) {
      // spuSaleAttrValueList 當前屬性的所有屬性值列表
      // spuSaleAttrValue 代表點擊的那一個屬性值
      // 排它第一步：讓所有成員全部都變成同一種狀態
      spuSaleAttrValueList.forEach((item) => (item.isChecked = "0"));
      // 排它第二步：讓當前點擊的這一個成員變為另一種狀態
    ...
    async addShopCart() {
      //第一步：先發請求添加購物車
      //dispatch英文意思：觸發分發的意思，本質就是在調用相關的action函數
      //現在我想要再組件這裡知道請求是成功還是失敗的
      // 我們只能通過dispatch的返回值，來判斷請求是成功的還是失敗的
      //目前我們下面這行代碼，結果一定是promise

      // 第一種寫法：action的異步函數，返回的永遠是成功的，要看action異步函數的action
      ...
      // 第二種寫法：action的異步函數，返回的promise可能成功也可能失敗，成功代表請求成功
      // 返回結果ok，失敗就拿到return的失敗的promise的原因
      try {
        //請求成功
        ...
        //往添加購物車成功跳轉的時候，需要帶兩個東西，skuNum和商品詳情訊息
        //skuNum是一個簡單數據，我們直接通過路由傳參query參數帶過去
        //商品詳情訊息是一個複雜的數據，我們最好不要通過路由傳參傳遞對象，因為我們採用存儲方案，sessionStorage
        alert("添加購物車成功，準備前往成功頁面");
  ...
  computed: {
    ...
    imgList() {
      //如果直接這樣寫，很可能是undefined，
      //當數據請求回來之前，這個值是undefined。
      //我們也必須在這裡或一個[]。
      //如果skuInfo.skuImageList是一個undefined，那麼我們應該給一個[]，
      //確保這個imgList，最後不是undefined。
